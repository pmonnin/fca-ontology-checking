...
def main():
    ...
    parser.add_argument("ontology_base_uri", help="Ontology base URI")
    ...
    with open(args.trilattice, 'r') as trilattice:
        for i, current_line in enumerate(trilattice):
            ...
            for d, value in enumerate(triconcept):
                triconcept[d] = value.split(',' + args.ontology_base_uri)

            if len(triconcept[dimension]) == 1 and triconcept[dimension][0] == "ø":
                context[str(i)] = []
            else:
                context[str(i)] = []
                for el in triconcept[dimension]:
                    if not el.startswith(args.ontology_base_uri):
                        el = args.ontology_base_uri + el
                    context[str(i)].append(el)
